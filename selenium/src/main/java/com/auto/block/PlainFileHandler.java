...
public class PlainFileHandler {
        ...
        try (RandomAccessFile raf = new RandomAccessFile(file,"r")){
           long rafLength = raf.length();
                ...
                for(int i=0;i<readSize;i++){
                    if(bs[i]==LF||(rafLength-1L)==i){
                        ...
                        if(lineNumber % fixNum==0){
                            ...
                            shardingBlock.setBolckIndex(batchNum);
            ...
            if(CollectionUtils.isNotEmpty(shardingBlockList)){
                ...
                lastBlock.setExtParam(JSONObject.toJSONString(fileLineData));
    ...
    public static List<String> nioShardingRead(long pointer,int nextLineNum,String localFilePath){
        List<String> shardingResult = Lists.newArrayList();
         File file = new File(localFilePath);
        if(file==null ||!file.exists()){
            log.info("本地路径{}下文件不存在",localFilePath+" ,从"+nextLineNum+"行开始,游标位置为"+pointer+",往后读取"+nextLineNum+"行数据处理");
            throw  new BizException("文件不存在");
        }

        log.info("开始读取文件 "+localFilePath+" 从"+nextLineNum+"行开始,游标位置为"+pointer+",往后读取"+nextLineNum+"行数据处理");
