...
import com.auto.service.core.campus.component.ConvertEntity;
...
@Slf4j
@Component
public class EcnusoleUniversityHandlerNew implements CampusOnlineHandler {
    ...
    public void onlineProcess(UserInfo userInfo, WebDriver driver, int course) throws Exception {
        ...
        for (int i = course; i < 10; i++) {
            //step :2进入学习      /html/body/div[3]/div[2]/div[1]/div[1]/a
            ...
            if (WebDriverUtils.check(driver, By.xpath(courseName))) {
                ...
                log.info("当前处理的课程:{}", titleName);
                //step3：点击进去
                driver.findElement(By.xpath(courseName)).click();
                Thread.sleep(2000);

                //step4：开启新页面 进去Ifame
                WebDriverUtils.switchToWindowByTitle(driver, titleName);
                Thread.sleep(5000);
                String ifame = "/html/body/iframe";
                WebDriverUtils.findElement(driver, ifame, "进入iframe");
                WebElement element = driver.findElement(By.xpath(ifame));
                driver.switchTo().frame(element);

                //step5:点击导学 切换界面
                String guidance = "/html/body/div[1]/div/div[2]/div[2]/div[2]/div[2]/ul/li[1]/div/div/div[2]";
                driver.findElement(By.xpath(guidance)).click();
                Thread.sleep(4000);
                //step6 :切换界面
                ...
                //开始界面处理
                oneStudyHandler(driver, titleName);
     ...
     * @param titleName
     */
    private void oneStudyHandler(WebDriver driver, String titleName) throws Exception {

        String contents = "/html/body/div[4]/div[3]/div[2]/div[1]/div/ul/li[1]/div[2]/ul/li[";
        for (int i = 0; i < 50; i++) {
            String contUrl = contents + i + "]/div/span[1]";
            String toDoList = contents + i + "]/div/span[2]/span[2]";
            String toDoListValue = driver.findElement(By.xpath(toDoList)).getText();
            log.info("toDoListValue:{}", toDoListValue);
            ConvertEntity convertEntity = convertToInt(toDoListValue);
            //若转化异常
            if (convertEntity.isFlag()) {
                //1.需要处理 还需要去考虑 视频 和 非视频的情况

                //2.根据
                String text = driver.findElement(By.xpath(contUrl)).getText();
                boolean flag = caseWork(text)? practiceHandler(driver,contUrl):viedosHandler(driver ,contUrl,convertEntity.getTodoInt());
            }
     ...
     * @return
     */
    private boolean practiceHandler(WebDriver driver,String contUrl) {
     ...
     * @return
     */
    public boolean viedosHandler(WebDriver driver,String contUrl,int todoInt) throws Exception {
        //1.点击 该课程
        driver.findElement(By.xpath(contUrl)).click();
        Thread.sleep(3000);
        log.info("视频处理逻辑");
        chainHandler( todoInt,  driver);
     ...
     *  将黄色点 转化为int 类型
     */
    public ConvertEntity convertToInt(String viedos) {
        ConvertEntity.ConvertEntityBuilder builder = ConvertEntity.builder();
        try {
           return  builder.todoInt(Integer.parseInt(viedos)).flag(true).build();
        } catch (Exception e) {
            return builder.flag(false).build();
        }
    }
     ...
     *  是否作业
     */
    public boolean caseWork(String text) {
        return  ("巩固练习".equals(text) || text.contains("巩固练习"))?true:false;
    }
...
//    }
    ...
    public EnumUniversityName universityName() {
        return EnumUniversityName.ECNUSOLE_UNIVERSITY_NEW;
    }
}
